...
```{python}
ex = allfiles["train.csv"][allfiles["train.csv"]["controlArea"]==1].head(250000)
halflife='0.5 min'
...
def features(d, i=None):
    cor_aligned = aligner(d, "correctionEcho", "correction", skip=5)
    smoothed = smoother(d.loc[:,['correctedDemand','aFRRactivation', 'aFRRrequest', 'BandLimitedCorrectedDemand', 'Datum_Uhrzeit_CET']]).reset_index(drop=True)
    ...
    windowed = window_analysis(normalized_smoothed)
    windowed = windowed.set_axis(["_".join(c) for c in windowed.columns],axis=1)
    res = pd.concat([cor_aligned, req_aligned, activation_aligned, normalized_smoothed, windowed],axis=1)
    if i is not None:
        res['i'] = i
    res['id'] = d.index
    return res
...
```{python}
sequencelen = int(round(2*60*60/4))
speed = 60
...
res = pd.concat([features(d,i) for i, d in tqdm(enumerate(win), total=wins)], axis=0)
```

```{python}
sizes = res.i.value_counts()
res = res[res.i.isin(sizes.index[sizes == sequencelen])].copy()
```
...
pipe = Pipeline([('scaler', StandardScaler()),('imputer', KNNImputer(n_neighbors=25, weights="uniform")),  ('outlier', IsolationForest(random_state=0, verbose=10, n_estimators=1000, max_samples=1024*8, max_features=1.0, bootstrap=True, n_jobs=2))], verbose=True).fit(res.drop(columns=["i", "id"]))
```

```{python}
outliers = pd.Series(pipe.predict(res.drop(columns=["i", "id"])))
outliers_groups = outliers.ne(outliers.shift()).cumsum()[outliers==-1]
g = outliers_groups.rename('group').reset_index(drop=False).groupby("group").indices
```

```{python}
toget = list(g.values())[90]
dat = allfiles["train.csv"][allfiles["train.csv"]["controlArea"]==1].copy()

ixbad = np.where(dat.index.isin(res["id"].iloc[toget]))
